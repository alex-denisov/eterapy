...
## ⚠ Новые поля состояния хоста нельзя проверить на staging

`/api/ops/node-status` **не отдаёт файл коллектора как есть** — он разбирает
его своим `parseNodeState` и отдаёт уже типизированный объект. Значит любое
новое поле (например `replication` из B537) срезается на стороне **ноды**,
пока сама нода крутит старый образ, — сколько бы свежей ни была панель.

Проверено на живом примере: коллектор на eterapy-1 писал
`replication: {role: primary, replicas: [eterapy2 streaming]}`, а нода на
образе `f6368164` отдавала `host` без этого ключа вовсе.

Практический вывод: **staging-проверка новой секции панели ничего не
доказывает**. Панель на staging покажет новое поле только после того, как
новый образ приедет на сами ноды, то есть после мержа в `main` и прод-деплоя.
На staging проверяйте отсутствие регрессий в уже существующих секциях, а новую
секцию — на проде после выката.
