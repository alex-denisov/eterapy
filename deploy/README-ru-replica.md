...
## ⛔ Незакрытый риск: деплой сломает standby

`deploy/compose/docker-compose.yml` (по нему разливаются eterapy-2/3/4):

- `migrate` выполняет `prisma migrate deploy` → на read-only реплике **упадёт**,
  а `web` зависит от него через `service_completed_successfully` → не поднимется;
- `worker` c `restart: always` → снова крэш-луп.

**Первый же мерж в main уводит eterapy-2 в красное.** До мержа нужно
（спека для следующего шага):

1. `migrate` — не выполнять миграции, если `FLEET_NODE_STANDBY=1`
   (маркер уже проставлен в `/opt/eterapy/.env` на eterapy-2).
   Выходить `0`, чтобы `depends_on` у `web` остался выполнимым.
2. `worker` — увести за профиль (`profiles: ["worker"]`), в
   `deploy/fleet-matrix.json` дать `--profile worker` узлам eterapy-3/4
   и пустую строку eterapy-2.
3. Тест-гард на матрицу — что standby-узел не получает worker-профиль.
