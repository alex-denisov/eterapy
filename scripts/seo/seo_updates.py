...
# This toolkit keeps its data next to the scripts; upstream keeps it one level
# up, at the plugin root. Prefer local, fall back to upstream so a file synced
# straight from upstream still resolves.
_DATA_FILE = next(
    (p for p in (_HERE / "data" / "google-updates.json",
                 _HERE.parent / "data" / "google-updates.json") if p.is_file()),
    _HERE / "data" / "google-updates.json",
)
