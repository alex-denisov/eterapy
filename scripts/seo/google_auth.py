...
# Least privilege, and narrower than upstream on purpose. We ship no Analytics
# and no Indexing-API tool, so we ask for neither: the consent screen must not
# request access this toolkit cannot use. Read-only Search Console covers both
# Search Analytics (gsc_query) and URL Inspection (gsc_inspect).
