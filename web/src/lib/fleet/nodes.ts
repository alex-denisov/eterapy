 ...
 *
 * Источник истины — переменная окружения `FLEET_NODES` (JSON-массив) в
 * `/opt/eterapy/.env`. В репозитории адресов нет намеренно (B539): узлы
 * описываются только на хосте.
 *
 * Пример:
