...
const DEFAULT_AI_TASK_POLICY_DEFINITIONS: AITaskPolicyDefinition[] = [
  ...
  {
    // B678 — отдельная политика, а не ветка «Ежедневной практики». У практики в
    // промте прямо записано «без мистики», и трактовка Таро под её правилами
    // была бы либо нарушением своего же промта, либо выхолощенным текстом.
    // Отдельная строка ещё и делает механику видимой в AI-центре суперадминки:
    // расход, провайдер и отказы карты дня считаются сами по себе.
    feature: "daily-tarot",
    enabled: true,
    tier: "free",
    title: "Карта дня (Таро)",
    purpose: "Трактовка карты дня в системе Райдера—Уэйта—Смит: 156 текстов, каждый генерируется один раз и кэшируется.",
    providerOrder: [...freeOrder],
    maxTokens: 500,
    temperature: 0.7,
    timeoutMs: 25_000,
    fallbackNotes: "Free-тир; при сбое — значение колоды детерминированным текстом, карта дня не пустует никогда.",
  },
  {
