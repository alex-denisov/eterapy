...
function synastryFactsForAI(wheel: SynastryWheel): string {
  ...
  return [
    "ТОЧНО ПОСЧИТАНО (используй как расчетный каркас и не выдумывай иных точных позиций):",
    `Солнце первого человека: ${wheel.a.sunSign.name}. Солнце второго: ${wheel.b.sunSign.name}.`,
    `Связей «где течёт»: ${flow}; «где трение»: ${tension}.`,
    "Опирайся на знаки Солнца обоих и баланс течения/трения. Не вставляй в текст результата общие дисклеймеры о том, что астрология не дает однозначных ответов.",
  ].join("\n");
}
