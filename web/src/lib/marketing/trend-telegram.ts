...
/**
 * B719 — СПИСОК КАНАЛОВ ЖИВЁТ В КОДЕ, А НЕ ТОЛЬКО В НАСТРОЙКЕ.
 *
 * Владелец 2026-08-23: «список телеграм-каналов я хочу чтобы ты составил сам.
 * Поищи самые популярные, подходящие по твоим критериям».
 *
 * Умолчание в коде, а не строка в базе, по правилу владельца от 2026-07-22:
 * значение, живущее только в ручной настройке, до прода не доезжает и
 * теряется при любом сбросе. Настройка `MARKETING_TREND_TELEGRAM_CHANNELS`
 * остаётся ПЕРЕОПРЕДЕЛЕНИЕМ: заполнена — берётся она, пуста — этот список.
 *
 * ⚠ КРИТЕРИИ ОТБОРА. Каждый канал проверен обращением к `t.me/s/<канал>` с
 * ноды, у которой есть доступ к Telegram, 2026-08-23. Проверялось:
 *
 *   1. ОТКРЫТ — веб-версия ленты отдаёт посты. Закрытый канал делает источник
 *      молча пустым, а не сломанным, и это худший вид отказа. Так отсеяны
 *      `taroe`, `podsoznanie_you`, `NumerologLiana2525`: у всех троих в
 *      рейтинге по 78–100 тыс. подписчиков, а веб-версия пуста.
 *   2. ЖИВ — последний пост не старше недели. `neo_goroskop` (79 тыс.)
 *      отвергнут не по числу постов, а по строке в самой ленте: «The owner of
 *      this channel has been inactive for the last 11 months».
 *   3. ПЛОТЕН — не меньше восьми постов в окне веб-версии. Тема считается по
 *      числу РАЗНЫХ постов (`TELEGRAM_TREND_MIN_POSTS`), поэтому редкий канал
 *      не даёт сигнала вовсе. По этому порогу отсеян и `daragan_konstantin`
 *      (209 тыс., сильная астрология) — три поста за две недели.
 *   4. В НАШИХ КЛАСТЕРАХ — расчёты по дате рождения, таро, астрология,
 *      психология отношений. Мотивация, книги и «саморазвитие вообще» дают
 *      биграммы, под которые у нас нет ни услуги, ни страницы.
 *   5. БЕЗ РЕКЛАМНОГО ШУМА. `astrologia_znaki_zodiaka` (110 тыс.) и
 *      `zvezdanutyygoro` (91 тыс.) отвергнуты потому, что свежие посты у них —
 *      реклама казино и конспектов ЕГЭ: эта лексика ушла бы в тренды как тема.
 *   6. РАЗНООБРАЗИЕ. Двадцать лент ежедневного гороскопа схлопнули бы все
 *      биграммы в «гороскоп день». Поэтому таро, астрология, расчёты по дате
 *      рождения и психология отношений держатся в списке рядом.
 *
 * ⚠ ЧЕГО В СПИСКЕ НЕТ И ПОЧЕМУ ЭТО ВАЖНО ЗНАТЬ. Матрица судьбы — наш самый
 * крупный кластер спроса, а крупных ОТКРЫТЫХ каналов именно по ней в
 * рейтингах не нашлось: тему ведут внутри общих эзотерических лент. Её несут
 * `st_ezoterika` и `anael_numerolog`, и это единственные две точки. Если
 * планировщик перестанет находить темы матрицы — причина здесь, а не в коде.
 */
export const TELEGRAM_TREND_DEFAULT_CHANNELS = [
  // Расчёты по дате рождения — наш главный кластер, и он же самый узкий.
  "st_ezoterika",            // ASTASY · матрица судьбы и нумерология
  "anael_numerolog",         // Марияна Анаэль · нумерология, 71 тыс.
  "natanlayakarta",          // натальная карта, 56 тыс.
  "YourHumanDesignRu",       // дизайн человека — единственный живой открытый
  // Таро.
  "astrogiks",               // astro geeks, 312 тыс. — крупнейший в рейтинге
  "tarogks",                 // tarot geeks · таро на день, 90 тыс.
  "lykova_taro",             // Анастасия Лыкова, 104 тыс.
  "taro1",                   // Расклады Таро, 99 тыс.
  "taro_jennifer",           // Карты, Джен и 2 кота, 79 тыс.
  // Астрология.
  "nehgoro",                 // нехилый гороскоп, 202 тыс.
  "mylablife",               // астролог Павел Андреев, 100 тыс.
  "astrologiya_ezoterika_kosmos",
  "kosmo_woman22",           // KOSMO_WOMAN, 70 тыс.
  "planetazemlyaastro",      // события неба: затмения, полнолуния, 64 тыс.
  // Широкая эзотерика — ею закрываются темы, не попавшие в узкие ленты.
  "ForMagicLife_RU",         // Анастасия Анисимова, 140 тыс.
  "magiaezoterica",          // гороскопы, таро, эзотерика, 58 тыс.
  // Психология отношений — наш вход в услуги, а не отдельная тема.
  "psikhologiak",            // Психология любви · отношения
  "otnoshenia_psi",          // Ты и Я · наука отношений
  "pcollege_EvaSneg",        // Ева Снежинская про отношения
  "abdartartemka",           // абдарт · психология расставаний, 130 тыс.
] as const;
...
export async function telegramChannelTrends(input: {
  ...
  const configured = channelsFrom(await marketingPlatformValue(TELEGRAM_TREND_CHANNELS_KEY));
  const channels = input.channels
    ?? (configured.length > 0 ? configured : [...TELEGRAM_TREND_DEFAULT_CHANNELS]);
