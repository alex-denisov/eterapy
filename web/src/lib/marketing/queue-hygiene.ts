...
export function planQueueHygiene(input: {
   ...
   *
   * Живой случай: строка неподключённой площадки со слотом 10.08 в статусе
   * SCHEDULED перебиралась публикатором каждую минуту неделю подряд и каждую
   * минуту отбивалась «коннектор не настроен».
   *
  ...
  for (const row of rows) {
    ...
    if (row.hasDraftText || isApproved(row)) {
      ...
      //
      // B713 §6: но просроченная дата живой не бывает. Без `!stale` строка со
      // слотом недельной давности проходила ровно здесь и возвращалась в
      // перебор — каждую минуту, неделю подряд.
