...
/**
 * Дата, с которой поступления вообще являются деньгами.
 *
 * До 2026-07-22 на проде лежат 33 «успешные» транзакции ЮKassa на 55 тысяч ₽ за
 * май–июнь. Денег по ним не приходило никогда: мерчантом ЮKassa платформа не
 * была, это следы тестового рельса (владелец 2026-07-22: «отключай Юкассу
 * навсегда», B570). Если их не отсечь, первая же выгрузка бухгалтеру принесёт
 * несуществующий доход — а налог платится с того, что в выгрузке.
 *
 * Отсечка — ДАННЫЕ с основанием, как и всё остальное здесь: появится импорт
 * настоящей истории, дата поменяется одной строкой.
 */
export const CASH_RAIL_START = {
  at: new Date(Date.UTC(2026, 6, 22)),
  basis: "запуск денежного рельса Robokassa на проде (B423); до этой даты платформа "
    + "не была мерчантом ни одной кассы — успешные транзакции ЮKassa за май–июнь тестовые",
};
...
export function buildIncomeBook(
  ...
  const entries: IncomeBookEntry[] = [...records]
    ...
    .map((item) => {
      if (item.recognizedAt < CASH_RAIL_START.at) {
        return {
          ...item,
          recognition: null,
          ownIncomeKopecks: null,
          issue: "До запуска денежного рельса: платформа не была мерчантом кассы, "
            + "деньги по этой строке не поступали",
        };
      }
