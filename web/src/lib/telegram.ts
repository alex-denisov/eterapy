...
export async function configureTelegramBot({ miniAppUrl, staging }: { miniAppUrl: string; staging: boolean }) {
  ...
  // Предел не 64 символа, а ЭКРАН ТЕЛЕФОНА: в строке поиска Telegram имя
  // обрезает кнопка «Открыть», которая сразу запускает мини-апп. Владелец
  // намерил 2026-08-13, что помещается около 21 символа. Поэтому имя короткое,
  // без хвоста бренда: бренд и так виден в @eterapy_bot.
  //
  // Первое слово отличается от канала («Таро и матрица судьбы — разборы ·
  // eTerapy») намеренно: в списке результатов видно именно первое слово, и
  // одинаковое начало заставило бы бота конкурировать с нашим же каналом.
  // «Матрица» одна, без «судьбы», читается двусмысленно — владелец заметил, и
  // Wordstat это подтверждает: рядом с ней стоят «матрица 2» (44 620),
  // «матрица воскрешение» (28 117), «матрица Ансоффа», «единичная матрица».
  // «Астрология» (235 773) такой двусмысленности не имеет и накрывает
  // натальную карту (958 760).
  //
  // Порядок слов не косметика: канал начинается с «Таро», и бот, начинающийся
  // так же, конкурировал бы с ним в списке результатов.
  const name = staging ? "Астрология и Таро (Stage)" : "Астрология и Таро";
