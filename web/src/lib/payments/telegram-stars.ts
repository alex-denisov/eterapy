 ...
 *
 * Округление ВНИЗ — owner-решение 2026-07-22 («нужно чтобы округлялось в
 * меньшую сторону»). Звезда целая, поэтому цена почти никогда не делится
 * нацело, и остаток надо кому-то отдать. Разница не больше одной звезды
 * (≈ курс, то есть единицы рублей) и достаётся покупателю: на входе в платный
 * рельс дешевле недобрать копейки, чем показать цену выше прейскуранта.
 *
 * Пол в одну звезду сохраняется: Telegram отвергает инвойс на 0 XTR, а
 * нулевая цена означала бы бесплатную выдачу платной услуги.
 */
export function starsForKopecks(amountKopecks: number, rate: number): number {
  ...
  return Math.max(MIN_STARS, Math.floor(rubles / rate));
}
