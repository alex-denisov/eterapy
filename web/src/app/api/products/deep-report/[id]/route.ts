...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.productResult.findFirst({
    where: { id, userId, productKey: "deep-report", deletedAt: null },
  });
  if (!result) return errorWithRequestContext("NOT_FOUND", "Deep report not found", 404, context);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!result) return errorWithRequestContext("NOT_FOUND", "Deep report not found", 404, context);
  if (result.status !== "READY") return errorWithRequestContext("CONFLICT", "Можно сохранить только готовый отчет", 409, context);
