...
export function RequisitesTab({ data }: { data: RequisitesTabData }) {
  ...
  return (
    <div className="mt-5 flex flex-col gap-4" data-testid="practitioner-finance-requisites">
      ...
      {/* B583 (решение владельца 2026-07-26): магазин Robokassa ОБЯЗАТЕЛЕН.
          Выплата идёт сплитом Robokassa, а сплит адресуется только на магазин
          Robokassa получателя — выплат по банковским реквизитам у Robokassa нет
          вовсе (ответ поддержки 2026-07-27). Магазин специалист заводит сам:
          платформа его не создаёт и создать не может.

          Блок показывается ВСЕГДА, а не после подтверждения статуса: раз без
          магазина выплаты не будет, узнать об этом надо в начале, а не в конце.
          Банковские реквизиты остаются — они нужны фискальной части и ручной
          выплате. */}
      <section>
        <p className="soft-eyebrow mb-2.5">Магазин Robokassa</p>
        <Link
          href={appUrl("/practitioner/finance/requisites/edit")}
            ...
            {data.payoutDetails?.robokassaAccount ? (
              <>
                ...
                <span className="mt-0.5 block text-xs text-[var(--soft-ink-faint)]">
                  Ваша доля приходит сюда сплитом в момент оплаты клиентом
                </span>
              </>
            ) : (
              <>
                <span className="block text-[13.5px] font-medium">Магазин не указан — выплаты недоступны</span>
                <span className="mt-0.5 block text-xs leading-relaxed text-[var(--soft-ink-faint)]">
                  Заведите магазин в Robokassa самостоятельно и укажите здесь его
                  идентификатор («Мои магазины» → «Настройка»). Ваша доля приходит
                  сплитом только на магазин Robokassa: выплат по банковским
                  реквизитам платёжная система не делает вовсе.
                </span>
              </>
            )}
          </span>
