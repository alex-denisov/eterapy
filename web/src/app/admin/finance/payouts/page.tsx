...
export default async function FinancePayoutsPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6" data-testid="admin-finance-payouts">
      ...
      >
        {/* B583. Ответ поддержки Robokassa 2026-07-27 уточнил рельс: холд и
            сплит НЕ совмещаются, а выплат по реквизитам («массовые выплаты») у
            провайдера нет вовсе. Значит автоматический путь ровно один — сплит
            в момент оплаты; всё остальное это ручной перевод. Панель
            показывает состояние и готовность, а не распоряжается деньгами:
            писать «селективные выплаты» значило бы обещать управление,
            которого нет. */}
        Комиссии практиков, KYC реквизитов, удержания и резерв по спорным операциям.
        Автоматически долю доводит только сплит — в момент оплаты клиентом и
        только на магазин Robokassa специалиста. Всё остальное — ручной перевод:
        выплат по банковским реквизитам Robokassa не делает. Здесь видно
        готовность к сплиту и учёт, но не управление переводом.
      </AdminHero>
