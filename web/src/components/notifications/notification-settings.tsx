  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        // Manual override for dev/testing if needed
        const status = {
          ...d,
          url: d.url?.replace("eterapy_deploy_bot", "eterapy_bot")
        };
            ...
            <div className="shrink-0 text-right">
              {!tgLinkUrl ? (
                <button onClick={generateTelegramLink} disabled={generatingLink}
                  className="rounded-lg border border-[var(--soft-paper-edge)] bg-[var(--soft-lilac-soft)] px-4 py-2 text-xs font-semibold text-[var(--soft-bordeaux)] hover:bg-[var(--soft-apricot)] transition-colors disabled:opacity-50">
                  {generatingLink ? "Загрузка..." : "Привязать Telegram"}
                </button>
              ) : (
                <div className="flex flex-col items-end gap-2" data-testid="telegram-link-pending">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-medium text-[var(--soft-sage)] animate-pulse">Ожидаем запуск бота...</span>
                    <a href={tgLinkUrl} target="_blank" rel="noopener noreferrer"
                      className="rounded-lg border border-[var(--soft-paper-edge)] bg-[var(--soft-lilac-soft)] px-4 py-2 text-xs font-semibold text-[var(--soft-bordeaux)] hover:bg-[var(--soft-apricot)] transition-colors inline-flex items-center gap-2">
                      Открыть Telegram
                    </a>
                  </div>
