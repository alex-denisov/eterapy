...
export function MiniAppProductActions({
  ...
  if (product.slug === "family-questions") return <FamilyScenariosActions creditCost={product.creditCost ?? 4} />;
  if (product.slug === "human-design") return <HumanDesignActions creditCost={product.creditCost ?? 2} />;
  if (product.slug === "surname-origin") return <SurnameStoryActions creditCost={product.creditCost ?? 2} />;
  return null;
}
