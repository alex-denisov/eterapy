...
export function MiniAppNotificationSettingsScreen() {
  ...
  async function save() { setState("saving"); try { const response = await fetch("/api/notifications/preferences", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prefs, quietHours: quiet }) }); setState(response.ok ? "saved" : "error"); } catch { setState("error"); } }
...
export function MiniAppInvitesScreen() {
  ...
  async function createLink() { setState("loading"); try { const response = await fetch("/api/referral/link", { method: "POST" }); const payload = await response.json().catch(() => ({})) as { url?: string }; if (response.ok && payload.url) { setUrl(payload.url); setState("idle"); } else setState("error"); } catch { setState("error"); } }
