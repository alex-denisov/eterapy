...
import { ArrowLeft, ArrowRight, Bell, CheckCircle, Copy, DownloadSimple, Envelope, LinkSimple, Lock, ShieldCheck, Trash, User } from "@phosphor-icons/react";
...
function Head({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return <header className={styles["subpage-head"]}><Link href="/miniapp/profile" className={styles["subpage-back"]}><ArrowLeft size={17} />Назад</Link><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p>{description}</p></header>;
}
...
export function MiniAppSecuritySettingsScreen() {
  ...
  return <MiniAppChrome data={data}><div className={styles.subpage}><Head eyebrow="безопасность" title="Способы входа" description="Внутри Telegram вход работает по подписанному Telegram ID. На сайте — только email и пароль." />{data.viewer.authenticated ? <><section className={styles["settings-hero"]}><ShieldCheck size={25} /><div><small>TELEGRAM MINI APP</small><strong>Сквозной вход активен</strong></div></section><form className={styles["account-form"]} onSubmit={submit}><label><span>Текущий пароль</span><input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></label><label><span>Новый пароль</span><input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" required minLength={8} /></label><label><span>Повторите новый пароль</span><input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" required minLength={8} /></label>{message ? <p className={message === "Пароль изменён" ? styles["flow-note"] : styles["form-error"]}>{message}</p> : null}<button className={styles["journey-primary"]} type="submit" disabled={saving}>{saving ? "Сохраняем…" : "Изменить пароль"}<Lock size={18} /></button></form><Link className={styles["journey-secondary"]} href="/miniapp/account/recover">Не помню текущий пароль</Link></> : <GuestGate />}</div></MiniAppChrome>;
}

type Pref = { event: string; category?: string; channel: "EMAIL" | "TELEGRAM" | "WEB"; enabled: boolean; remindBeforeHours: Array<number | null> };
