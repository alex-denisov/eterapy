...
export function CheckinExperience({
  ...
  const inMiniApp = surface === "miniapp";
  const productPath = (href: string) => inMiniApp ? toMiniAppPath(href) : href;
  ...
  const accountPath = (mode: "login" | "register", intent: string) => {
    if (!inMiniApp) return `/${mode}?intent=${encodeURIComponent(intent)}`;
    const returnTo = `/miniapp/checkin${dialogue?.id ? `?dialogueId=${encodeURIComponent(dialogue.id)}` : ""}`;
    return `/miniapp/account?mode=${mode}&intent=${encodeURIComponent(intent)}&returnTo=${encodeURIComponent(returnTo)}`;
  };
        ...
        <div className="soft-answer-flow" data-testid="dialogue-result-step">
          ...
          <section className="mt-5" data-testid="dialogue-answer-triage-layout" aria-label="Что можно сделать дальше">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 sm:items-stretch" data-testid="dialogue-triage-rail">
              ...
              <button
                ...
                data-analytics-target={productPath(`/products/chat?dialogueId=${dialogue.id}`)}
                ...
                onClick={() => {
                  ...
                  if (status !== "authenticated") {
                    navigate(inMiniApp
                      ? `/miniapp/account?mode=login&intent=continue-dialogue&returnTo=${encodeURIComponent(toMiniAppPath(next))}`
              ...
              {productRecommendation && productRecommendation.slug !== "reframe" ? (
                <Link
                  href={productPath(`${productRecommendation.href}?dialogueId=${dialogue.id}`)}
                  className="soft-card soft-triage-primary flex flex-col p-5"
                  data-testid="triage-primary-cta"
                  data-analytics-surface="checkin_triage"
                  data-analytics-event="triage_primary_clicked"
                  data-analytics-target={productPath(`${productRecommendation.href}?dialogueId=${dialogue.id}`)}
                ...
                <Link
                  href={productPath("/products/reframe")}
                  className="soft-card soft-triage-primary flex flex-col p-5"
                  data-testid="triage-primary-cta"
                  data-analytics-surface="checkin_triage"
                  data-analytics-event="triage_primary_clicked"
                  data-analytics-target={productPath("/products/reframe")}
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2" data-testid="triage-secondary-options">
              ...
              {secondaryProducts.map((item) => {
                ...
                const href = productPath(`${item.href}?dialogueId=${dialogue.id}`);
                return (
                  <Link
                    key={item.slug}
                    href={href}
              ...
              {recommendations.length > 0 ? (
                recommendations.slice(0, 1).map((rec) => (
                  <Link
                    ...
                    data-analytics-surface="checkin_triage"
                    data-analytics-event="triage_secondary_clicked"
                    data-analytics-target={productPath(`/practitioners/${rec.slug}?dialogueId=${dialogue.id}`)}
                ...
                <Link
                  href={productPath(`/practitioners?dialogueId=${dialogue.id}`)}
                  className="soft-triage-option sm:col-span-2 ring-1 ring-[var(--soft-terracotta-dark)] bg-[var(--soft-paper-card)]"
                  data-testid="specialist-recommendation"
                  data-analytics-surface="checkin_triage"
                  data-analytics-event="triage_secondary_clicked"
                  data-analytics-target={productPath(`/practitioners?dialogueId=${dialogue.id}`)}
                  data-analytics-product="specialist"
                  data-analytics-dialogue-id={dialogue.id}
                  data-analytics-cta-role="secondary"
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2" data-testid="dialogue-free-continuation-actions">
            {status === "authenticated" ? (
              ...
              <Link href={accountPath("login", "save-result")} className="soft-button soft-button-primary" data-testid="save-result-login">
                <Bookmark className="size-4" aria-hidden="true" />
                Сохранить в карту
              </Link>
            )}
          ...
          {status !== "authenticated" && (
            <p className="mt-4 text-sm text-[var(--soft-ink-soft)]">
              ...
              <Link href={accountPath("login", "save-result")} className="font-medium text-[var(--soft-terracotta-dark)] hover:underline">
                Войти
              </Link>
            </p>
          )}
            ...
            {limitPaywall.cta === "register" ? (
              <>
                ...
                <div className="mt-5 flex flex-col gap-2">
                  <Link
                    href={accountPath("register", "continue-dialogue")}
                  ...
                  <Link href={accountPath("login", "continue-dialogue")} className="soft-button soft-button-soft justify-center">
                    Войти
                  </Link>
