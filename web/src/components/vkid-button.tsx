...
export function VKIDButton(
  { disabled = false, onBeforeAuth }: { disabled?: boolean; onBeforeAuth?: () => void } = {},
) {
  ...
  return (
    <button
      ...
      className={`soft-social-button flex items-center justify-center gap-2 rounded-full px-3 py-2.5 text-sm font-semibold transition-colors ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
    >
