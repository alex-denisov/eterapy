...
export function CabinetResultRow({ item }: { item: CabinetResultRowItem }) {
  ...
  return (
    <div className="soft-result-row" data-testid="cabinet-result-row">
      ...
      <Link href={item.href} className="soft-result-body block" data-testid="cabinet-result-open">
        <span className="soft-result-title block">{item.title}</span>
        <span className="soft-result-date block">{item.when}</span>
      </Link>
      <div className="soft-result-acts">
        ...
        >
          {hiding ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <EyeOff className="size-4" aria-hidden="true" />}
        </button>
      </div>
    </div>
  );
