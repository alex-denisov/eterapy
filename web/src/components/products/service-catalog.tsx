...
const GROUPS: ServiceGroup[] = [
  {
    // B672 · триаж перестаёт называться порядковым номером. «Первый разбор»
    // читался как обязательный шаг маршрута — владелец 2026-08-05: «все
    // нажимают эту кнопку, потому что думают, что начинать нужно с неё».
    // Заголовок группы условный: тот, кто знает, что ему нужно, сам себя из
    // неё исключает.
    id: "free",
    nav: "Сориентироваться",
    title: "Если пока не знаете, что выбрать",
    layout: "entry",
    cards: [
      { id: "primary", title: "Понять, что дальше", desc: "Расскажите, что происходит, — увидите ситуацию со стороны и поймёте, что дальше.", price: "0 ₽", href: "/checkin", icon: Heart },
    ],
  },
  {
    // B673 (владелец 2026-08-05): чип «Разборы» → «Психология», а «Вместе»
    // перестаёт быть отдельной категорией из одной карточки и переезжает сюда.
    // Заголовок секции меняется вместе с чипом: «Разобраться самостоятельно»
    // стало бы неправдой над карточкой, где участников двое.
    id: "solo",
    nav: "Психология",
    title: "Психология",
    layout: "grid",
    cards: [
      ...
      // B674: «Вместе» не сообщало ни предмета, ни механики — до второго
      // участника мысль просто не доходила. «Двоих» — счётное слово, а не тип
      // отношений: мать, взрослый сын и подруга в него попадают, а «парный»
      // сузил бы услугу до романтической пары.
      { id: "together", title: "Разбор для двоих", desc: "Каждый отвечает отдельно — итог общий", price: `от ${price("pair")}`, href: "/products/pair", icon: Users },
    ],
  },
  {
    id: "esoteric",
