...
describe("B620 · разметка ленты", () => {
  ...
  it("планка ленты — наша, а не площадки", () => {
    // B698: прежняя подпись называла это «порогом площадки». Порог у Дзена — 10
    // ПОДПИСЧИКОВ канала, и к числу материалов в ленте он не относится. Ошибка
    // была не косметической: на «пороге площадки» висели два ускорителя,
    // обещавших выключиться «на десятом материале» — обещание стало
    // невыполнимым, когда выпуск ушёл в браузер и лента перестала расти.
    expect(DZEN_FEED_TARGET_ITEMS).toBe(10);
  });
});
