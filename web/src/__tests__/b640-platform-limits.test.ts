...
describe("B640 — нарушение возвращается на доработку, а не в архив", () => {
  ...
  /**
   * B713 §2 изменил половину этого правила по требованию владельца 2026-08-17.
   *
   * Было: на не-последнем раунде длина выносилась замечанием наружу, «у автора
   * ещё есть раунд, чтобы сократить осмысленно». Замер прода 03.08–17.08
   * показал, что он не сокращает: 27 смертей материалов на превышении лимита,
   * включая «5240 символов при лимите 1000» ПОСЛЕ двух раундов правки. Модель
   * не умеет считать символы и после прямого запрета — замечание о длине
   * превращалось в бесконечный круг.
   *
   * Стало: длину снимает код на каждом раунде, до подсчёта замечаний.
   * Визуальная идея остаётся замечанием: её нельзя посчитать, её надо
   * придумать, и это работа автора, а не регулярки.
   */
  it("не последний раунд: длину снял код, визуальная идея вынесена наружу", () => {
    // B705: площадка сменена на VK — у Telegram визуальная идея перестала быть
    // обязательной, и вид `media-brief` там больше не возникает.
    ...
    expect(result.violations
      ...
      .sort())
      .toEqual(["media-brief"]);
    // Текст усечён кодом уже на первом раунде: редактор его длинным не увидит.
    expect(result.draft.text.length).toBeLessThanOrEqual(1_500);
