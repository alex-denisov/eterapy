...
describe("B582 — запрос на прогноз опознаётся как эзотерический", () => {
  ...
  it("ловит «звёзды подскажут» — найдено на живых диалогах прода", () => {
    expect(detectClarifierRegister("может звезды подскажут мне что делать")).toBe("symbolic");
    expect(detectClarifierRegister("Может, звёзды подскажут?")).toBe("symbolic");
  });

  it("но не считает эзотерикой любое упоминание звёзд", () => {
    // Маркер — звёзды, которые ПОДСКАЗЫВАЮТ, а не слово «звезда».
    expect(detectClarifierRegister("Отель на пять звёзд, а я всё равно не отдохнул")).toBe("psychological");
    expect(detectClarifierRegister("Моя коллега ведёт себя как звезда эстрады")).toBe("psychological");
  });
