...
describe("B699 · срок остывания берётся у провайдера", () => {
  ...
  describe("классификация ключа", () => {
    ...
    // B705: срок поднят с часа до двенадцати. Замер прода 2026-08-12 — 31 отказ
    // CEREBRAS за двое суток и ни одной генерации: 402 снимает пополнение счёта
    // владельцем, и до этого момента каждая проба гарантированно бесплодна.
    it("402 у Cerebras — исчерпанный счёт, ключ возвращается дважды в сутки", () => {
      expect(classifyCredentialFailure("HTTP_402").cooldownMs).toBe(12 * HOUR);
    });
