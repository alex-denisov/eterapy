/**
 * B525 — юридический аудит пакета (переработка в рамках редакции 1.0).
 *
...
describe("B425 — индивидуальное решение по диспуту", () => {
  it("не обещает автоматическую формулу частичного возврата", () => {
    const sessions = legalDocMarkdown("sessions");
    const pack = source("src/content/legal-pack.md");

    expect(sessions).toContain("индивидуально согласованном");
    expect(sessions).toContain("нет заранее установленной формулы");
    expect(pack).toContain("нет фиксированной формулы или автоматического процента");
    expect(pack).toContain("возвращаемая сумма списывается с долей получателей");
  });
});
