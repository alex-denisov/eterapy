...
describe("B702 фаза 3 — планировщик тем слота", () => {
  ...
  it("одно общее слово трендом статью не делает", () => {
    // Живой прогон источников на проде 2026-08-11 вернул обрывки вроде
    // «личного бренда» и «основе старших». Если тренд засчитывается по ОДНОМУ
    // совпавшему слову, такой обрывок поднимает случайную статью на верхний
    // ярус и обходит спрос — тренд обязан совпасть целиком.
    const planned = plan({
      slots: [slot("t1")],
      trends: [{ topic: "тренд посторонний", rationale: "", source: "discovery", keywords: ["тренд", "посторонний"] }],
      usedByPlatform: new Map(),
    });
    expect(planned.get("t1")?.origin).toBe("core");
  });
